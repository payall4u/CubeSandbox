...
| S5.5d.2 durable create、持久化与 Host cgroup 快路径 | `IN_PROGRESS` | Codex | 合并可恢复 journal 阶段并移除固定 placement 等待 | 不关闭 fsync/精确回读；Shim create→start-vm 串行/并发 P95≤140/150ms；原子故障矩阵通过；累计 CRI→VMM≤220/300ms；10 Pod absolute start 展开≤250ms |
