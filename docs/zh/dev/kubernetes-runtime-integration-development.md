...
| S5.5d.2 durable create、持久化与 Host cgroup 快路径 | `IN_PROGRESS` | Codex | d.2a/d.2b 已完成并分别经 reviewer PASS：移除固定 placement 等待、合并相邻 WAL，并以 schema v2 将四个 Host controller 的正常 journal 提交从 10 次降为 2 次；旧 schema 可恢复，第三值 durable DEGRADED | d.2c 固定 commit/artifact 首次执行 50 串行与 5×10 并发：Shim create→start-vm P95≤140/150ms、CRI→VMM≤220/300ms、10 Pod absolute start 展开≤250ms；功能/故障/exact-zero 回归与证据复验通过后关闭 |
