...
| S5.5 普通冷启动性能优化 | `IN_PROGRESS` | Codex | S5.5a～d.2b 已完成且 reviewer `PASS`；d.2c-r1 串行门禁通过、并发性能 FAIL；d.3a-r2 已用完整源码完成可靠性复验并由 reviewer `PASS`（P0/P1/P2=0/0/0） | [S5.5 性能方案](./kubernetes-runtime-performance-s5.5.md)；[S5.5a 基线](../../handoffs/kubernetes-runtime/evidence/s5.5/s5.5a-final-baseline.md)；[S5.5d.3 证据](../../handoffs/kubernetes-runtime/evidence/s5.5/s5.5d.3-network-vmm-overlap.md) | 按用户优先级进入 S5.5e Guest 冷启动/内存；d.3b/c 延后 |
...
| S5.5d.3 network/VMM 解耦与重验 | `DEFERRED` | Codex | d.3a-r1 因旧源码 base 无效；r2 使用完整 `52976d3b` 重建，5×10 首次 CRI 50/50、neighbor error=0、旧 WAL 慢路径=0、13 项 exact-zero，reviewer `PASS`（P0/P1/P2=0/0/0）；d.3b/c 延后 | [S5.5d.3 证据](../../handoffs/kubernetes-runtime/evidence/s5.5/s5.5d.3-network-vmm-overlap.md)；当前进入 S5.5e；Guest 优化后按并发剩余瓶颈决定是否恢复 d.3b/c |
| S5.5e Guest 冷启动与普通路径内存优化 | `IN_PROGRESS` | Codex | 把 Agent/vsock 提前并缩短 kernel/init 关键路径；删除无收益的 Guest/worker 私有常驻页 | VMM→vsock 串行 P95≤950ms、并发≤1050ms；同口径 20 Pod 稳态 Host cgroup 均值≤100MiB/Pod，且节点 MemAvailable 差值摊销相对 S5.5a 下降≥15%或≤100MiB/Pod；能力冒烟零回退 |
